	...
	methods: {
		...
		toggleDone: function (event, id) {
			...
			if(task) {
				...
				Api.updateTask(this.task).then(function(response){
					app.listTasks();
					app.clear();
					let status = response.completed ? 'completed(true)' : 'in pr(false)';
					app.message = `GenBar ${response.id} is ${status}`;
				})
		...
		createTask: function(event) {
			...
			Api.createTask(this.task).then(function(response){
				app.listTasks();
				app.clear();
				app.message = `GenArticle ${response.id} created`;
			})
		...
		updateTask: function(event, id) {
			...
			Api.updateTask(this.task).then(function(response){
				app.listTasks();
				app.clear();
				app.message = `GenArticle ${response.id} update`;
			})
		...
		deleteTask: function(event, id) {
			...
			if(taskIndex > -1) {
				Api.deleteTask(id).then(function (response){
					...
					app.message = `GenArticle ${id} deleted`;
				})
			}
		}
	},
